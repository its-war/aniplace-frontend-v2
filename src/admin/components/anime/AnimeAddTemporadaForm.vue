<template>
  <div class="temporada-container">
    ...
    <div class="temporada-formulario" style="width: 80%; margin: 20px auto">
      <h5>Obs: O sistema se encarrega de colocar o número da temporada e os números dos episódios, então certifique-se de cadastrar tudo de forma sequencial.</h5>
      <v-card>
        <v-card-text>
          <v-textarea label="Cole os links dos episódios em ordem sequencial separando-os por uma quebra de linha (enter)"
                      v-model="links"
                      rows="6"
                      @paste="colarEvent"
                      variant="outlined"
        ...
        <v-card-actions>
          <v-spacer/>
          <v-btn @click="cadastrarTemporada" :loading="loading" append-icon="mdi-send" variant="outlined" text="Cadastrar" color="success"></v-btn>
        </v-card-actions>
      </v-card>
    </div>
...
<script>//TODO: exibir o numero de temporadas atual e deixar claro que uma nova vai ser criada.
export default {
